...
@section('header', 'PANEL HUMAN RESOURCES')

@section('content')

    {{-- GLOBAL BACKGROUND GRID EFFECT (ORANGE THEME) --}}
    <div class="fixed inset-0 pointer-events-none z-0" 
         style="background-image: linear-gradient(rgba(249, 115, 22, 0.05) 1px, transparent 1px), 
    ...
    <div class="holo-card rounded-xl overflow-hidden relative border border-orange-500/20 bg-[#0B1120]/90 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.5)] mb-8 z-10">
        
        {{-- Header Terminal --}}
        <div class="bg-black/40 border-b border-orange-500/10 p-3 px-6 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-orange-400 font-mono font-bold text-xs tracking-[0.2em] ml-4 opacity-80">
                    DASHBOARD
                </h3>
            </div>
            <div class="text-[10px] font-mono text-orange-500/50 animate-pulse">
                ONLINE
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 relative z-10">
        ...
        {{-- CARD 1: PERAN ANDA --}}
        <div class="holo-card rounded-lg p-6 border border-orange-500/30 bg-[#0f172a]/80 backdrop-blur-sm relative group overflow-hidden hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(249,115,22,0.2)] transition-all duration-300">
            {{-- HUD Corners --}}
            <div class="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 border-orange-400"></div>
            <div class="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-orange-400"></div>
            
            <div class="absolute -right-8 -top-8 w-24 h-24 bg-orange-500/10 rounded-full blur-2xl group-hover:bg-orange-500/20 transition-all duration-500"></div>
            
            <div class="flex flex-col h-full justify-between relative z-10">
                <div class="flex items-start justify-between mb-4">
                     <div class="h-12 w-12 rounded border border-orange-500/30 flex items-center justify-center text-orange-400 bg-black/30 shadow-[0_0_15px_rgba(249,115,22,0.1)]">
                        <i class="fas fa-user-tie text-xl"></i>
                    </div>
                    <span class="text-[10px] font-mono text-orange-500/50">ROLE</span>
                </div>
                
                <div>
                    <p class="font-mono text-[10px] text-orange-400 tracking-[0.2em] uppercase mb-1">Peran Anda</p>
        ...
        @php
            ...
            // Logic Warna: Jingga/Merah jika belum, Hijau Neon jika sudah
            ...
            $icon        = $hasEvaluated ? 'fa-check-circle' : 'fa-exclamation-circle';
            $statusText  = $hasEvaluated ? 'SUDAH MENILAI' : 'BELUM MENILAI';
        @endphp
            ...
            <div class="absolute -left-8 -bottom-8 w-24 h-24 {{ $bgGlow }} rounded-full blur-2xl transition-all duration-500"></div>
             ...
             <div class="flex flex-col h-full justify-between relative z-10">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span class="text-[10px] font-mono {{ $hasEvaluated ? 'text-emerald-500/50' : 'text-red-500/50' }}">STATUSES</span>
                </div>
